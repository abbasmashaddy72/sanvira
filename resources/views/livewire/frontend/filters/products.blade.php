<div>
    @if ($type === 'Category Page' || $type === 'All Category Page')
        @if (count($sub_category) !== 0 || $this->type === 'All Category Page')
            <x-frontend.index-container class="py-14">
                @if ($type === 'All Category Page' && count($sub_category) >= 20)
    ...
    @if ($type !== 'All Category Page' && $type !== 'Product Details Similar Products')
        <x-frontend.index-container class="{{ $type !== 'Profile Page' ? 'py-14' : '' }}"
            containerTitle="{{ $page_title }}" containerSlot="{{ $products->total() }} Products">
