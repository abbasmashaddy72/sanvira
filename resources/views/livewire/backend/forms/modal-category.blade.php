<x-backend.modal-form form-action="add" title="{{ $name }}">
    <div class="row-gap-0 grid-cols-1 gap-2 sm:grid">
        <div class="flex space-x-6">
            <x-radio id="main-category" label="{{ __('Main Category') }}" wire:model.live="type" value='Main Category'
                required />
            <x-radio id="sub-category" label="{{ __('Sub Category') }}" wire:model.live="type" value='Sub Category' required />
        </div>
        ...
        @if ($type == 'Sub Category')
            <x-select label="{{ __('Select Parent / Sub Category') }}" wire:model="parent_id"
