    ...
    <div class="grid grid-cols-2 gap-x-2">
        ...
        <div class="card">
            <x-backend.image-upload :images="$this->images" :isUploaded="$this->isUploaded"
                label="{{ __('Upload Images') }}" name="images" deletId="{{ $supplier_product_id }}" model="images"
                required />
        </div>

        <div class="card">
            <x-backend.file-upload :files="$this->data_sheets" :isUploaded="$this->isDataSheetsUploaded"
                label="{{ __('Upload Datasheets') }}" name="data_sheets" deletId="{{ $supplier_product_id }}"
                model="data_sheets" />
        </div>

        <x-select label="{{ __('Select Category') }}" wire:model="main_category_id" placeholder="Select Category"
            :async-data="route('api.admin.supplier_categories', ['parent_id' => 0])" option-label="name"
            option-value="id" required />

        @if ($sub_category)
        <x-select label="{{ __('Select Subcategory') }}" wire:model.defer="supplier_product_category_id"
            placeholder="Select Subcategory"
            :async-data="route('api.admin.supplier_categories', ['parent_id' => $main_category_id])" option-label="name"
            option-value="id" required wire:change="updateSelectedSubcategory" />
        @endif
