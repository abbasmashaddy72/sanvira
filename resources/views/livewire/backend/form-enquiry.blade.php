    ...
    <form wire:submit.prevent="submit">
        <div class="grid grid-cols-6 gap-2">
            <div class="col-span-6 rounded-lg bg-white px-4 dark:bg-gray-800 sm:p-6">
                <p class="text-lg font-semibold">Enquiry Details</p>
                <hr class="my-2 h-px border-0 bg-gray-200 dark:bg-gray-700" />
                <div class="grid grid-cols-5 items-end gap-x-2 gap-y-4">
                    <x-input name="rfq_no" label="{{ __('RFQ No.') }}" type="text" value='{{ $data->rfq->rfq_no }}'
                        disabled />
                    ...
                    <x-input name="enquiry_no" label="{{ __('Enquiry No.') }}" type="text"
                        value='{{ $data->enquiry_no }}' disabled />

                    <x-input name="rfq_rfq_submission_date_time" label="{{ __('RFQ Submission Date Time') }}"
                        type="text" value='{{ $data->rfq_submission_date_time->toDayDateTimeString() }}' disabled />
