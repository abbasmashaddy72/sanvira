    ...
    <form wire:submit.prevent="submit">
        <div class="grid grid-cols-6 gap-2">
            <div class="col-span-4 rounded-lg bg-white px-4 dark:bg-gray-800 sm:p-6">
                <p class="text-lg font-semibold">Delivery Note Details</p>
                <hr class="my-2 h-px border-0 bg-gray-200 dark:bg-gray-700" />
                <div class="grid grid-cols-5 items-end gap-x-2 gap-y-4">
                    <x-input name="order_no" label="{{ __('Order No.') }}" type="text"
                        value='{{ $data->order->order_no }}' disabled />

                    <x-input name="usre_name" label="{{ __('Buyer Name') }}" type="text"
                        value='{{ $data->buyer->name }}' disabled />

                    <x-input name="delivery_note_no" label="{{ __('Delivery Note No.') }}" type="text"
                        value='{{ $data->delivery_note_no }}' disabled />

                    <x-input name="order_submission_date_time" label="{{ __('Order Submission Date Time') }}"
                        type="text" value='{{ $data->order_submission_date_time->toDayDateTimeString() }}'
                        disabled />
            ...
            <div class="col-span-2 flex h-full flex-col rounded-lg bg-white px-4 dark:bg-gray-800 sm:p-6">
                <div>
                    <p class="text-lg font-semibold">Deliery Notes Attachments</p>
                    <hr class="my-2 h-px border-0 bg-gray-200 dark:bg-gray-700" />
                </div>

                <div class="mt-auto">
                    <x-input label="Delivery Notes PDF" placeholder="Only PDF" wire:model='delivery_notes_attachment'
                        type='file' />
                </div>
            </div>
