    ...
    <form wire:submit.prevent="submit">
        <div class="grid grid-cols-6 gap-2">
            <div class="col-span-6 rounded-lg bg-white px-4 dark:bg-gray-800 sm:p-6">
                <p class="text-lg font-semibold">Invoice Details</p>
                ...
                <div class="grid grid-cols-5 items-end gap-x-2 gap-y-4">
                    <x-input name="delivery_note_no" label="{{ __('Delivery Note No.') }}" type="text"
                        value='{{ $data->deliveryNote->delivery_note_no }}' disabled />

                    <x-input name="usre_name" label="{{ __('Buyer Name') }}" type="text"
                        value='{{ $data->buyer->name }}' disabled />

                    <x-input name="invoice_no" label="{{ __('Invoice No.') }}" type="text"
                        value='{{ $data->invoice_no }}' disabled />

                    <x-input name="delivery_note_submission_date_time"
                        label="{{ __('Delivery Note Submission Date Time') }}" type="text"
                        value='{{ $data->delivery_note_submission_date_time->toDayDateTimeString() }}' disabled />
