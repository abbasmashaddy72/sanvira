...
<div class="{{ $spacing }} {{ $zIndex }} fixed inset-0 overflow-y-auto" x-data="wireui_modal({
    show: @toJs($show),
    @if ($attributes->wire('model')->value()) model: @entangle($attributes->wire('model')).live @endif
})"
    ...
    {{ $attributes->whereDoesntStartWith('wire:model.live')->whereStartsWith(['x-on:', '@', 'wire:']) }} style="display: none"
    ...
    <div class="{{ $align }} {{ $maxWidth }} mx-auto flex min-h-full w-full transform items-end justify-center"
        x-show="show" x-on:click.self="close" x-transition:enter="ease-out duration-300"
