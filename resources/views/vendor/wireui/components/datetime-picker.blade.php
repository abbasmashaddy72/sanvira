        ...
        <div x-show="tab === 'date'" class="space-y-5">
            @unless ($withoutTips)
                <div class="text-secondary-600 grid grid-cols-3 gap-x-2 text-center">
                    <x-dynamic-component :component="WireUi::component('button')" class="bg-secondary-100 dark:bg-secondary-800 border-none"
            ...
            <div class="relative">
                <div class="dark:bg-secondary-800 absolute inset-0 grid grid-cols-3 gap-3 bg-white"
                ...
                <div class="grid grid-cols-7 gap-2">
                    <template x-for="day in weekDays" :key="`week-day.${day}`">
                        <span class="text-secondary-400 text-3xs pointer-events-none text-center uppercase"
                            x-text="day">
                        </span>
                    </template>
        ...
        <div x-show="tab === 'time'" x-transition>
            ...
            <div x-ref="timesContainer"
                ...
                <template x-for="time in filteredTimes" :key="time.value">
                    <button
                        class="focus:bg-primary-100 dark:focus:bg-secondary-700 hover:bg-primary-600 dark:hover:bg-secondary-700 dark:text-secondary-400 group relative cursor-pointer select-none rounded-md py-2 pl-2 pr-9 text-left transition-colors duration-100 ease-in-out hover:text-white focus:outline-none"
                        :class="{
