<div class="{{ $align }} {{ $zIndex }} fixed inset-0 flex items-end justify-center overflow-y-auto sm:pt-16"
    ...
    <div class="bg-secondary-400 {{ $dialog }}-backdrop @if ($blur) {{ $blur }} @endif dark:bg-secondary-700 fixed inset-0 transform bg-opacity-60 transition-opacity dark:bg-opacity-60"
    ...
    <div class="w-full p-4 transition-all sm:max-w-lg" x-show="show" x-transition:enter="ease-out duration-300"
        ...
        <div class="dark:bg-secondary-800 relative space-y-4 rounded-xl bg-white p-4 shadow-md"
            :class="{
                'sm:p-5 sm:pt-7': style === 'center',
                'sm:p-0 sm:pt-1': style === 'inline',
            }">
            <div class="bg-secondary-300 dark:bg-secondary-600 absolute left-0 top-0 rounded-full transition-all duration-150 ease-linear"
            ...
            <div x-show="dialog && dialog.closeButton" class="absolute -top-2 right-2">
                <button
                    class="{{ $dialog }}-button-close focus:ring-secondary-200 text-secondary-300 rounded-full p-1 focus:outline-none focus:ring-2"
            ...
            <div class="space-y-4"
                ...
                <div class="mt-4 w-full" :class="{ 'sm:mt-5': style === 'center' }">
                    <h3 class="text-secondary-900 dark:text-secondary-400 text-center text-lg font-medium leading-6"
                        :class="{ 'sm:text-left': style === 'inline' }"
                        @unless ($title) x-ref="title" @endunless>
                        {{ $title }}
                    </h3>

                    <p class="text-secondary-500 mt-2 text-center text-sm"
                        :class="{ 'sm:text-left': style === 'inline' }"
