...
@else
    <div @class([$theme->baseClass]) style="{{ $theme->baseStyle }}">
        @if (!$inline)
            <label class="text-pg-primary-700 dark:text-pg-primary-300 block text-sm font-medium">
                {{ $title }}
            </label>
        @endif
        <div @class(['pt-1' => !$inline, 'relative'])>
            <select class="{{ $filterClasses }}" style="{{ data_get($column, 'headerStyle') }}"
            ...
            <div
                ...
                <x-livewire-powergrid::icons.down class="h-4 w-4 dark:text-gray-300" />
            </div>
        </div>
    </div>
@endif
